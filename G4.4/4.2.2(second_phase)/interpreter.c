...
volatile int counter_global = 0, l = 0, noftests, num = 0, threads_counter = 0;
volatile pthread_mutex_t mtx;
volatile pthread_cond_t cond_mtx;
...
void *run_thread(int *num_thread) {
    ...
    int isArg = 0, s = 0, r = 0, p = 0, q = 0, isprint;
    ...
    do{
        ...
        /*=============================*/
        /*threads_counter is for the case we have only one thread*/
        if(threads_counter != 1){
            if( *num_thread != 1){
                ...
                *num_thread++;
                ...
                if (pthread_cond_wait((pthread_cond_t*)&cond_mtx, (pthread_mutex_t*)&mtx) != 0) {                                  
                ...
                if(noftests == 0){
                    ...
                    if (pthread_cond_signal((pthread_cond_t*)&cond_mtx) != 0) {                                  
        ...
        /*here we print the contents after an instruction executed*/
        ...
        /*we see if there is other test here*/
        temp = node->next;
...
int main(int argc, char *argv[]) {
    ...
    if (pthread_cond_init((pthread_cond_t*)&cond_mtx, NULL) != 0) {                                    
    ...
    b = (int*)malloc(atoi(argv[1])*sizeof(int));
    if(b == NULL) {
        write(1, "allocation error\n", 17);
        exit(1);
    }
    ...
    do {
        ...
        if(c == '\n') {
            ...
            /*l used to init and then add the list*/
            if(l != 0) {
                add(head, curr, l);
                buf[l].flag = 0;
            }
            ...
            if(nofthreads <= atoi(argv[1])) {
                while( nofthreads <= atoi(argv[1]) ){
                    pthread_create(&thread_id, NULL, (void*)run_thread, (void*)&b[counter]);
                    counter++;
                    nofthreads++;
                }
            }
